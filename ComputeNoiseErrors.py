...
for problem in problems:
    for n in noise:
        ...
        for m in m_vec:
            if abl:
                string_to_exec = (
                    "sbatch --time=24:00:00 -n 1  --mem-per-cpu=24000 "
                    " --wrap=\" python3 ComputeErrorsAbl.py "
                    + str(problem)
                    + " "
                    + str(n)
                    + " "
                ...
                if problem != "rad":
                    ...
                    string_to_exec = (
                        "sbatch --time=24:00:00 -n 1 --mem-per-cpu=8000"
                        " --wrap=\" python3 PlotRad.py "
                        + str(n)
                        + " "
                        + str(m)
                        + " "
