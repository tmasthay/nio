...
for which in ["eit", "rad"]:
    for abl_fno in [True, False]:
        ...
        paths = [
            ...
            "/branch_architecture.txt",
            "/trunk_architecture.txt",
            "/fno_architecture.txt",
            "/denseblock_architecture.txt",
        ]
            ...
            with open(path + p) as f:
                for line in f:
                    ...
                    if (
                        "norm" not in key
                        and "act" not in key
                        and "scheduler" not in key
                        and "retrain" not in key
                    ):
        ...
        if abl_fno:
            script = "RunNio.py"
            folder_name = (
                "ModelSelection_final_abl_fno_conv_" + which + "_" + model
            )
        else:
            script = "RunNioAbl.py"
